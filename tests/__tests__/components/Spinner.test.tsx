...
describe("SpinnerComponent", () => {
  it("renders without crashing", () => {
    const { lastFrame } = render(<SpinnerComponent message="Loading..." />);
    expect(lastFrame()).toBeTruthy();
  });

  it("displays the message", () => {
    const { lastFrame } = render(<SpinnerComponent message="Please wait" />);
    expect(lastFrame()).toContain("Please wait");
  });

  it("truncates long messages", () => {
    const longMessage = "A".repeat(300);
    const { lastFrame } = render(<SpinnerComponent message={longMessage} />);
  ...
  it("handles empty message", () => {
    const { lastFrame } = render(<SpinnerComponent message="" />);
    expect(lastFrame()).toBeTruthy();
  });
  ...
  it("handles message just over max length", () => {
    ...
    const { lastFrame } = render(<SpinnerComponent message={overMaxMessage} />);
