...
describe("StatusBadge", () => {
  ...
  it("displays provisioning status", () => {
    const { lastFrame } = render(<StatusBadge status="provisioning" />);
    expect(lastFrame()).toContain("PROVISION");
  });

  it("displays suspended status", () => {
    const { lastFrame } = render(<StatusBadge status="suspended" />);
    expect(lastFrame()).toContain("SUSPENDED");
  });

  it("displays failure status", () => {
    const { lastFrame } = render(<StatusBadge status="failure" />);
    expect(lastFrame()).toContain("FAILED");
  });

  it("displays shutdown status", () => {
    const { lastFrame } = render(<StatusBadge status="shutdown" />);
