...
describe("DevboxCard", () => {
  it("renders without crashing", () => {
    const { lastFrame } = render(<DevboxCard id="dbx_123" status="running" />);
    expect(lastFrame()).toBeTruthy();
  });

  it("displays devbox id", () => {
    const { lastFrame } = render(
      <DevboxCard id="dbx_test_id" status="running" />,
    );
  ...
  it("displays devbox name when provided", () => {
    const { lastFrame } = render(
      <DevboxCard id="dbx_123" name="my-devbox" status="running" />,
    );
  ...
  it("shows provisioning status icon", () => {
    const { lastFrame } = render(
      <DevboxCard id="dbx_123" status="provisioning" />,
    );
    expect(lastFrame()).toContain("…"); // ellipsis for provisioning
  });

  it("shows failed status icon", () => {
    const { lastFrame } = render(<DevboxCard id="dbx_123" status="failed" />);
  ...
  it("displays created date when provided", () => {
    ...
    const { lastFrame } = render(
      <DevboxCard id="dbx_123" status="running" createdAt={createdAt} />,
    );
  ...
  it("truncates long names", () => {
    ...
    const { lastFrame } = render(
      <DevboxCard id="dbx_123" name={longName} status="running" />,
    );
