/**
 * Test setup for Router and integration tests.
 * Same as setup-components.ts but does NOT mock navigationStore so that
 * NavigationProvider and Router use real navigation state.
 */
...
// Note: We do NOT mock navigationStore - Router tests need real NavigationProvider
...
// Mock zustand stores (Router calls getState().clearAll() on route change)
const mockClearAll = jest.fn();
const mockStoreWithClear = () => ({
  devboxes: [],
  loading: false,
  error: null,
...
jest.mock("../src/store/devboxStore.ts", () => ({
  useDevboxStore: Object.assign(jest.fn(mockStoreWithClear), {
