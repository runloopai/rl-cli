...
export const DevboxCreatePage = ({
  ...
  // Agent picker
  if (showAgentPicker) {
    // Show crown column when public agents may appear (public tab, or merged search in private tab)
    const showCrown = agentPickerTab === "public" || agentPickerMerged;
    const buildAgentColumns = (tw: number): Column<Agent>[] => {
      const fixedWidth = 6;
      const crownWidth = showCrown ? 3 : 0;
      const idWidth = 25;
      ...
      const cols: Column<Agent>[] = [
        createTextColumn<Agent>("id", "ID", (agent) => agent.id, {
          width: idWidth + 1,
          color: colors.idColor,
          dimColor: false,
          bold: false,
        ...
        cols.push(
          createComponentColumn<Agent>(
            ...
            (agent, _index, isSelected) => {
              const isPublic =
                agent.is_public || (agent.name || "").startsWith("➰ ");
              return (
      ...
      cols.push(
        ...
        createTextColumn<Agent>(
          ...
          "Created",
          (agent) =>
            agent.create_time_ms ? formatTimeAgo(agent.create_time_ms) : "",
          {
            width: timeWidth,
            color: colors.textDim,
            dimColor: false,
          ...
          config={{
            ...
            fetchPage: async (params) => {
              if (agentPickerTab === "private") {
                ...
                return {
                  items: publicResult.agents.map((a) => ({
                    ...a,
                    name: `➰ ${a.name}`,
                  })),
